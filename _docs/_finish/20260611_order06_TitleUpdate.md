...
- `TitleScene.js` の `init()`, `update(deltaTime)`, `draw(ctx, layerId)` の各メソッド内で、`initTitleAnimation()`, `updateTitleAnimation()`, `drawTitleAnimation()` などの必要な関数が正しく呼び出され、画面に宝石が落下し、爆散する美しいパーティクル演出が描画されるように結線を修復してください。
