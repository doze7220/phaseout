...
- 現在、`MasterRenderer` の `requestAnimationFrame` ループから `SceneManager.update(deltaTime)` へ経過時間が渡されていますが、`SceneManager.changeScene` や `pushScene` で重い初期化処理が走った直後のフレームで、巨大な `deltaTime` が流入するタイムスパイクが発生しています。
...
## 3. ドキュメントの更新
- 作業完了後、`changelog.js` のバージョンを `v0.11.10` に加算し、「シーン遷移直後の巨大なdeltaTimeによる物理演算のカクつき（タイムスパイク）を防止するリセット機構を追加」として記載してください。
