...
export class ScreenEffectTransition {
    ...
    drawGlobalPostEffects(ctx) {
        ...
        } else if (PhaseManager.getCurrentPhaseName() === 'ブラック解除演出中') {
            this.drawPhaseBlackExit(ctx, PhaseManager.stateTimer);
        }
        ...
        if (elapsed >= timeFlicker) {
            ...
            if (tribalAlpha > 0) {
                ...
                if (blackPhaseElapsed > 0) {
                    ...
                    // 鏃(やじり)十字の描画
                    if (drawP > 0) {
                        const wOffset = conf.WEDGE_OFFSET !== undefined ? conf.WEDGE_OFFSET : 7;
                        ...
                        const baseY = outerR - wHeight + wIndent + wOffset;
    ...
    drawPhaseBlackExit(ctx, elapsed) {
